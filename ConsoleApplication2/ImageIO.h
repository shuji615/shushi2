...
public:
	...
	template <class T>
	static bool loadMatImage(cv::Mat &mat,T*& pImagePlane,int& width,int& height, int& nchannels);
	template <class T>
...
template <class T>
bool ImageIO::loadMatImage(cv::Mat &im, T *&pImagePlane, int &width, int &height, int &nchannels)
{
	if(im.data == NULL) // if allocation fails
		return false;
	if(im.type()!= CV_8UC1 && im.type()!=CV_8UC3 && im.type()!=CV_8UC4) // we only support three types of image information for now
