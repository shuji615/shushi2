...
public:
	...
	virtual bool matimread(cv::Mat &mat);
#ifndef _MATLAB
...
/*
template <class T>
void Image<T>::copyMatData(const cv::Mat& mat)
{
	imWidth=mat.cols;
	imHeight=mat.rows;
...
template <class T>
bool Image<T>::matimread(cv::Mat &mat)
{
	clear();
	if(ImageIO::loadMatImage(mat,pData,imWidth,imHeight,nChannels))
	{
		computeDimension();
